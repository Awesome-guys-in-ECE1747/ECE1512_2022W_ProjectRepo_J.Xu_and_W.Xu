...
def layer_finder(k_model, model_arch, pool_input=True):
  ...
  '''
    ...
    if type(model_arch) != str:
        raise TypeError(
            "Input argument \"model_arch\" must be a string that is\
    ...
    if model_arch == "VGG":
        ...
        for i in range(len(k_model.layers)):
            ...
            if k_model.layers[i + 1].output.get_shape(
            )[2] < k_model.layers[i].output.get_shape()[2] - 4:
                if pool_input == True:
                    if type(k_model.layers[i]) == tf.keras.layers.InputLayer:
                        continue
                    if 'ZeroPadding2D' in str(type(k_model.layers[i])):
                        if type(k_model.layers[
                                i - 1]) == tf.keras.layers.InputLayer:
                            continue
                    ...
                    if first_layer:
                        ...
                        while (pool_flag):
                            ...
                            if 'merge.Add' in str(type(k_model.layers[j])):
                                ...
                                #print(j)
                            elif block_end_detected == True and 'Conv2D' in str(
                                    type(k_model.layers[j])):
                                #print('Here')
                                last_layers.append(k_model.layers[j - 1].name)
                                block_end_detected = False
                                pool_flag = False
        ...
        print(
            "Input argument \"model_arch\" must be either \"VGG\" or \"ResNet\"."
        )
...
def create_attribution_masks(img,
                             model,
                             layers,
                             class_index,
                             max_mask_num,
    ...
    assert interp in ['bilinear', 'nearest'
                      ], 'Selected upsampling type undefined or unsupported.'
    ...
    for i in range(len(layers)):
        ...
        if interp == 'bilinear':
            fmap = tf.image.resize(tmp_mask[..., idxs],
                                   size,
                                   method='bilinear').numpy()
        elif interp == 'nearest':
            fmap = tf.image.resize(tmp_mask[..., idxs], size,
                                   method='nearest').numpy()
        else:
            raise ValueError(
                'You have selected an unsupported interpolation type.')
...
def visualize_layers(img,
    ...
    if masks.shape[2] <= batch_size:
        preds_masked = np.append(preds_masked,
                                 model(X, training=False)[:, class_index],
                                 axis=0)
    else:
        for i in range(0, masks.shape[2] - batch_size, batch_size):
            preds_masked = np.append(preds_masked,
                                     model(X[i:i + batch_size],
                                           training=False)[:, class_index],
                                     axis=0)
        preds_masked = np.append(preds_masked,
                                 model(X[i + batch_size:],
                                       training=False)[:, class_index],
                                 axis=0)
...
def SISE(img,
    ...
    '''
    masks = create_attribution_masks(img,
                                     model,
                                     layers,
                                     class_index=class_index,
                                     max_mask_num=grad_thr,
                                     interp=interp)
    exmaps = []
...
def weighted_fusion(w, exmaps, T=100.):
    ...
    e23 = np.multiply(
        (exmaps[:, :, 0] * w_post[0] + exmaps[:, :, 1] * (2 - w_post[0])),
    ...
    e23456 = np.multiply(
        (e2345 * w_post[3] + exmaps[:, :, 4] * (2 - w_post[3])),
