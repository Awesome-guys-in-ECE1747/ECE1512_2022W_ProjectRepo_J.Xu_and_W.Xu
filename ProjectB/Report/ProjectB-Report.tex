...
However, as we can see here in the table, the hyperparameters do not affect the test accuracy too much. The variance is smaller than 1\%. So we can conclude that the adjustment of hyperparameter does not help a lot for the student models to improve their accuracy and mimic the predictions of the teacher model. It's highly possible that the difference is jitters of in the training.
  ...
  We choose RISE to do the visualization, the code is highly adapted from the $xai\_utils.py$ in Project A, but with modifications to support 1-Channel Images. Besides, the calling function is also modified to fit in the format of data. A here is the modified function of RISE.
    \begin{lstlisting}
...
\begin{figure}[h]
		\centering  %图片全局居中
\subfigure[Temperature=1]{
			\label{sum-1}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t1_acc.png}}
\subfigure[Temperature=2]{
			\label{sum-2}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t2_acc.png}}
\subfigure[Temperature=4]{
			\label{sum-4}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t4_acc.png}}
\subfigure[Temperature=16]{
			\label{sum-16}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t16_acc.png}}
		\subfigure[Temperature=32]{
			\label{sum-32}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t32_acc.png}}
		\subfigure[Temperature=64]{
			\label{sum-64}
			\includegraphics[width=0.4\textwidth]{./graphs/Task2/t64_acc.png}}
